...
// --- INÍCIO DA MODIFICAÇÃO PARA EVITAR QUEBRA HTTP ---
...
if (file_exists("functions.php")) {
   ...
   if (isset($_POST["action"]) && $_POST["action"] == "CheckLicense") {
    // 1. Resposta imediata para qualquer pedido de verificação (Bypass de rede)
    ...
    // Envia a resposta completa para o sistema
    echo json_encode($checkLicense);
    exit; 
}
    ...
    // MODULO HÍBRIDO: Detecta se é Link ou Usuário/Senha
    if (strpos($inputUname, 'http') === 0) { // É um link
        $urlParts = parse_url($inputUname);
        parse_str($urlParts['query'] ?? '', $query);
        $UserName = $query['username'] ?? "";
        $UserPassword = $query['password'] ?? "";
        $server = $urlParts['scheme'] . "://" . $urlParts['host'] . (isset($urlParts['port']) ? ":" . $urlParts['port'] : "");
    } else { // É login XC manual
        $UserName = $_POST["uname"];
        $UserPassword = $_POST["upass"];
        $server = $_POST["server"];
    }

    // Salva o DNS correto na sessão para carregar a Playlist depois
    ...
    if ($checkLogin["result"] == "success") {
        $userData = $checkLogin["data"]->user_info;
        $serverData = $checkLogin["data"]->server_info;

        // Monta a Sessão Padrão do Player
        ...
        echo json_encode(["result" => "error", "message" => "Dados Inválidos"]);
    }
    exit;
}
    if (isset($_POST["action"]) && $_POST["action"] == "CheckLicense") {
    ...
    if (isset($_POST["action"]) && $_POST["action"] == "getMoviesDataFromCategoryId") {
        ...
        $datalimit = isset($_POST["datalimit"]) ? $_POST["datalimit"] : 9999;
        $bar = "/";
        if (substr($hostURL, -1) == "/") {
            $bar = "";
        }
